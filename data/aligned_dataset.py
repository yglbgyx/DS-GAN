...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        #     A, B = T(A, B)
        # w2 = int(w / 2)
        # A = AB.crop((0, 0, w2, h)).resize((self.opt.loadSize_w, self.opt.loadSize_h), Image.BICUBIC)  #这里应该是切割数据集的图像，并且将图像放到指定的尺寸（这里是256*256）
        # B = AB.crop((w2, 0, w, h)).resize((self.opt.loadSize_w, self.opt.loadSize_h), Image.BICUBIC)

        A = transforms.ToTensor()(A)  #将PIL Image或者 ndarray 转换为tensor，并且归一化至[0-1] 注意事项：归一化至[0-1]是直接除以255，若自己的ndarray数据尺度有变化，则需要自行修改。
