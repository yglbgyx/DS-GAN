...
class WO_ConvNeXt(nn.Module):
    def __init__(self, inplans, planes, norm_layer=nn.InstanceNorm2d):
        super(WO_ConvNeXt, self).__init__()
        self.model = nn.Sequential(
            nn.Conv2d(inplans, planes, kernel_size=3, padding=1),
            norm_layer(planes),
            nn.GELU()
        )
...
class DSGAN_WO_ConvNeXt(nn.Module):
    def __init__(self):
        super(DSGAN_WO_ConvNeXt, self).__init__()
        # self.conv = nn.Conv2d(3, 64, kernel_size=7, padding=3)

        self.c1 = WO_ConvNeXt(3, 64)
        self.d1 = downSample()
        self.c2 = WO_ConvNeXt(64, 128)
        self.d2 = downSample()
        self.c3 = WO_ConvNeXt(128, 256)
        self.d3 = downSample()
        self.c4 = WO_ConvNeXt(256, 512)
        self.d4 = downSample()
        self.c5 = WO_ConvNeXt(512, 1024)

        self.u1 = upSample(1024, 512)
        self.uc1 = WO_ConvNeXt(1024, 512)
        self.u2 = upSample(512, 256)
        self.uc2 = WO_ConvNeXt(512, 256)
        self.u3 = upSample(256, 128)
        self.uc3 = WO_ConvNeXt(256, 128)
        self.u4 = upSample(128, 64)
        self.uc4 = WO_ConvNeXt(128, 64)
